    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(9,8,10,0.97)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      {submitted ? (
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 22, color: '#EEE5D5', fontWeight: 700 }}>{eveningDone ? t('whatWinnersDo') : t('tomorrowNewDay')}</div>
          <div style={{ fontSize: 14, color: '#4A4050', marginTop: 8 }}>{t('seeYouTomorrow', { name: agentName })}</div>
        </div>
      ) : (
        <div style={{ maxWidth: 460, width: '100%' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
            {([{ id: 'morning', label: `☀️ ${t('morning')}` }, { id: 'evening', label: `🌙 ${t('evening')}` }] as const).map(p => (
          ...
          {phase === 'morning' && (
            <div>
              <h3 style={{ fontSize: 20, color: '#EEE5D5', fontWeight: 700, marginBottom: 8, lineHeight: 1.4 }}>{t('oneThingFirstSale')}</h3>
              <div style={{ fontSize: 12, color: '#3A3040', marginBottom: 14 }}>{t('thisWeekLabel')} <span style={{ color: '#D4A853' }}>{weekAction}</span></div>
              <textarea value={morningQ} onChange={e => setMorningQ(e.target.value)} placeholder={t('writeItDown')} rows={3} style={{ width: '100%', background: '#100F14', border: '1px solid #2A2430', borderRadius: 12, padding: '14px', fontSize: 14, color: '#EEE5D5', outline: 'none', resize: 'none', lineHeight: 1.6 }} />
              <button onClick={() => morningQ.trim() && (saveCheckin({ morningAnswer: morningQ.trim() }), setPhase('locked'))} style={{ marginTop: 12, width: '100%', background: morningQ.trim() ? '#D4A853' : '#1A1820', color: morningQ.trim() ? '#09080A' : '#2A2430', border: 'none', borderRadius: 12, padding: '14px', fontSize: 15, fontWeight: 700, cursor: morningQ.trim() ? 'pointer' : 'default', transition: 'all 0.2s' }}>{t('commitToIt')}</button>
            </div>
          )}
          {phase === 'locked' && (
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 44, marginBottom: 12 }}>🎯</div>
              <h3 style={{ fontSize: 18, color: '#EEE5D5', fontWeight: 700, marginBottom: 10 }}>{t('lockedIn')}</h3>
              ...
              <p style={{ fontSize: 13, color: '#4A4050', marginBottom: 16 }}>{t('nowGoDoIt')}</p>
              <button onClick={onClose} style={{ background: 'transparent', border: '1px solid #2A2430', color: '#6A6070', borderRadius: 12, padding: '10px 24px', cursor: 'pointer', fontSize: 13 }}>{t('close')}</button>
            </div>
          )}
          {phase === 'evening' && (
            <div>
              <h3 style={{ fontSize: 20, color: '#EEE5D5', fontWeight: 700, marginBottom: 16 }}>{t('didYouDoIt')}</h3>
              <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
                {[{ v: true, label: `✅ ${t('yes')}`, color: '#6BAE94' }, { v: false, label: `❌ ${t('no')}`, color: '#E07B6A' }].map(o => (
              ...
              {eveningDone !== null && (
                <>
                  <textarea value={eveningNote} onChange={e => setEveningNote(e.target.value)} placeholder={eveningDone ? t('eveningDonePlaceholder') : t('eveningNotDonePlaceholder')} rows={3} style={{ width: '100%', background: '#100F14', border: '1px solid #2A2430', borderRadius: 12, padding: '14px', fontSize: 14, color: '#EEE5D5', outline: 'none', resize: 'none', lineHeight: 1.6, marginBottom: 12 }} />
                  <button onClick={submit} style={{ width: '100%', background: '#9B7EC8', color: '#09080A', border: 'none', borderRadius: 12, padding: '14px', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>{t('doneForToday')}</button>
                </>
              )}
            </div>
          )}

...
export default async function TrainingPage() {
  const t = await getTranslations('shell.training')
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#09080A', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ maxWidth: 420, textAlign: 'center', color: '#DDD5C8', fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
        <div style={{ fontSize: 11, letterSpacing: '0.3em', color: '#D4A853', textTransform: 'uppercase', marginBottom: 10 }}>{t('eyebrow')}</div>
        <h2 style={{ fontSize: 22, color: '#EEE5D5', marginBottom: 10 }}>{t('notAvailable')}</h2>
        <p style={{ fontSize: 14, color: '#6A6070', lineHeight: 1.6 }}>{t('roleOnly')}</p>
      </div>
    </div>
  )
}
